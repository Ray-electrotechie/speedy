...
CHARWIDE = 55
DEF_col_width = 300
#blue_col_size = (DEF_col_width,200)
...
window = None
#
Python_version = textwrap.fill('Python:'+sg.sys.version,width = CHARWIDE)
System_info = textwrap.fill('System:'+runningon.platform(),width = CHARWIDE)
Uname = textwrap.fill('System:'+sprint(runningon.uname()),width = CHARWIDE)
PySimpleGui_source = textwrap.fill('PySimpleGui source:'+sprint(sg),width = CHARWIDE)
PySimpleGui_version = textwrap.fill('PySimpleGui version:'+sg.ver,width = CHARWIDE)
TCL_version = textwrap.fill('TCL version:'+sg.tclversion_detailed,width = CHARWIDE)
global current_user
...
#
DEF_version = '2.7'
DEF_ANNOUNCE_program = 'Speedy Nordic nRF52840 Dongle Programmer '+DEF_version
...
#Below is crude and slow. I could not get async pipes to work without losing data.
#the result files are not enormous so using files is not too slow.
...
try:
    ...
    last_resort_log.exception('Fatal error - cannot open (or create) log file= '+user_log_file)
...
#A message can be declared critical so it cannot be cancelled but will still time out as normal
...
#   that the message will remain for at least the number of seconds declared at instantiation (then vanish) and
#   boolean important = True means that it cannot be cancelled (it must be replaced to remove it).
# timeout = False means that the message will remain until overwritten with a new message.
...
def log_event(str):
    ...
    speedy_logger.write('\n'+time.strftime("%d/%m/%Y %H:%M")+str) # and copy to the log
...
\nFiles can only be selected when not uploading and not in Auto', enable_events = True,
                     size=(18,4),font=('any',16,'bold'),button_color=browse_button_color,key = '-FILE-',disabled=False)],
                  [sg.Button('searching for\ndongle',size=(18,4),font=('any',16,'bold'),button_color='white on orange',key = '-ACTION-',disabled=True,
                  tooltip = 'When green push to program the dongle.\nDo not unplug dongle until told')],
                  [sg.Button('Auto is off',          size=(18,5),font=("any",16,"bold"),button_color='white on red', key = '-AUTO-',
...
\nthe first one found with a dongle\nwill be programmed, followed by the others')],
                  [sg.Text('          ',             size=(20,2),font=('any',16,'bold'),key='-STATE-')]
                 ], background_color='blue',key='-BLUE-',size=(300,460)
                ) #from layout helper funcs. Vtop align tops of [elements]
         ,sg.Col([
                  [sg.Text('Below a record of activity - also in log',justification='center', size =('50','2'))],
                  [sg.Multiline('Nordic Dongle Programmer version '+DEF_version+'\n',autoscroll = True,size = (CHARWIDE, 30),
                  key='-REPORT-',auto_refresh = True,write_only = True,tooltip = 'Report on activity. No user input here.')],
                  ], background_color='green',key='-GREEN-',size=(400,460))]
         )
         
         ,[sg.Button('Exit', key='-E-')]
         ]

# Display the window and get values
window = sg.Window(DEF_ANNOUNCE_program, layout = layout, resizable = True, margins=(0,0), element_padding=(0,0),finalize=True)
